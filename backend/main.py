...
def _write_analisis_ia(ws, run: CanonicalRun | None = None):
    ...
    plan_rows = []
    for p in (diag.get("analyst_review_plan") or [])[:10]:
...
def _run_ai_report_html(run: CanonicalRun, summary: dict[str, Any] | None = None, theme: str = "dark") -> str:
    ...
    hidden_kpis = {"CON REFERENCIA", "SIN REFERENCIA PLENA", "COBERTURA DE MERCADO", "ALERTAS CRITICAS", "ALERTAS CRÍTICAS"}
    kpi_cards = ''
    visible_kpis = [k for k in diag.get("kpis", []) if str(k.get("label", "")).strip().upper() not in hidden_kpis]
    for k in visible_kpis[:12]:
    ...
    if theme == "light":
        palette = ":root{--bg:#f6f8fc;--bg2:#eef3f9;--card:#ffffff;--card2:#f8fafc;--line:#d8e0ea;--text:#101828;--muted:#667085;--blue:#1877f2;--green:#12b76a;--yellow:#f79009;--red:#f04438;--shadow:0 18px 50px rgba(16,24,40,.10)}"
        hero_bg = "linear-gradient(135deg,#ffffff,#f8fafc)"
        body_bg = "linear-gradient(180deg,var(--bg),var(--bg2))"
        p_color = "#475467"
        table_head = "#eef4ff"
        btn_secondary = "#ffffff"
    else:
        palette = ":root{--bg:#07111f;--bg2:#0b1527;--card:#0f1d33;--card2:#132642;--line:#263850;--text:#eaf1fb;--muted:#9fb0c7;--blue:#67a8ff;--green:#4ade80;--yellow:#fbbf24;--red:#fb7185;--shadow:0 22px 70px rgba(0,0,0,.25)}"
        hero_bg = "linear-gradient(135deg,rgba(19,38,66,.96),rgba(15,29,51,.96))"
        body_bg = "radial-gradient(circle at top,#142844,#07111f 60%)"
        p_color = "#c7d2e5"
        table_head = "rgba(103,168,255,.13)"
        btn_secondary = "#17243a"
    css = f"""
    {{palette}}*{{box-sizing:border-box}}body{{margin:0;background:{{body_bg}};color:var(--text);font-family:Inter,Segoe UI,Arial,sans-serif}}.wrap{{max-width:1440px;margin:auto;padding:34px 34px 50px}}.hero,.panel{{background:{{hero_bg}};border:1px solid var(--line);border-radius:24px;padding:24px;box-shadow:var(--shadow)}}.hero{{display:block;padding-bottom:24px}}h1{{margin:6px 0 8px;font-size:36px}}h2{{font-size:20px;margin:0 0 14px}}p{{color:{{p_color}};line-height:1.56}}.eyebrow{{font-size:12px;text-transform:uppercase;letter-spacing:.14em;color:var(--blue);font-weight:900}}.chip{{display:inline-flex;align-items:center;border-radius:999px;padding:6px 10px;font-size:12px;font-weight:900}}.chip.ok{{background:rgba(74,222,128,.16);color:var(--green)}}.chip.warn{{background:rgba(251,191,36,.16);color:var(--yellow)}}.chip.bad{{background:rgba(251,113,133,.16);color:var(--red)}}.kpis{{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:18px 0}}.kpi{{background:var(--card);border:1px solid var(--line);border-radius:18px;padding:16px}}.kpi span{{display:block;font-size:12px;color:var(--muted);text-transform:uppercase;letter-spacing:.08em}}.kpi strong{{display:block;font-size:23px;margin-top:8px}}.kpi small{{display:block;color:var(--muted);margin-top:5px}}.grid{{display:grid;grid-template-columns:.85fr 1.15fr;gap:18px;margin-top:18px}}.stack{{display:grid;gap:18px;margin-top:18px}}.bar{{margin:12px 0}}.bar div{{display:flex;justify-content:space-between;color:var(--muted);font-size:13px}}.bar b{{color:var(--text)}}.bar i{{display:block;height:12px;background:var(--card2);border:1px solid var(--line);border-radius:999px;overflow:hidden;margin-top:6px}}.bar em{{display:block;height:100%;background:linear-gradient(90deg,var(--blue),var(--green))}}.table-wrap{{overflow:auto;border:1px solid var(--line);border-radius:16px;background:var(--card)}}table{{width:100%;border-collapse:collapse;font-size:13px}}th,td{{border-bottom:1px solid var(--line);padding:10px;text-align:left;vertical-align:top}}th{{background:{{table_head}};color:var(--text);white-space:nowrap}}tr:last-child td{{border-bottom:0}}.muted{{color:var(--muted)}}.actions{{display:flex;gap:12px;margin-top:18px;flex-wrap:wrap}}.btn{{background:#2563eb;color:white;text-decoration:none;border-radius:12px;padding:12px 16px;font-weight:900}}.btn.secondary{{background:{{btn_secondary}};border:1px solid var(--line);color:var(--text)}}.decision{{border-left:5px solid var(--yellow)}}.decision.green{{border-left-color:var(--green)}}.decision.red{{border-left-color:var(--red)}}@media(max-width:1000px){{.hero,.grid,.kpis{{grid-template-columns:1fr}}.wrap{{padding:18px}}}}
    """
    return f"""<!doctype html><html lang='es'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>{_html_escape(title)}</title><style>{css}</style></head><body><main class='wrap'><section class='hero'><div><div class='eyebrow'>{_html_escape(run_label)}</div><h1>{_html_escape(title)}</h1><p>{_html_escape(executive)}</p><div class='actions'><a class='btn' href='/api/real-runs/{_html_escape(run.run_id)}/report'>Descargar Excel</a><a class='btn secondary' href='#plan'>Ver plan de revisión</a></div></div></section><section class='kpis'>{kpi_cards}</section><div class='grid'><section class='panel'><h2>Participación por sección</h2>{bars or '<p class="muted">Sin desglose disponible.</p>'}</section><section class='panel'><h2>Resumen por secciones APU</h2>{section_table}</section></div><section class='stack'><section class='panel'><h2>Top 10 materiales por impacto</h2>{materials_table}</section><section class='panel'><h2>Top 10 mano de obra / cuadrillas</h2>{labor_table}</section><section class='panel'><h2>Top 10 maquinaria / equipo</h2>{equipment_table}</section><section class='panel'><h2>Partidas críticas del catálogo</h2>{concepts_table}</section><section class='panel'><h2>Alertas contra mercado</h2>{alerts_table}</section><section class='panel' id='plan'><h2>Plan de revisión para el analista</h2>{plan_table}</section><section class='panel'><h2>Diagnóstico profesional breve</h2>{diag_table}</section></section></main></body></html>"""
...
def real_run_ai_report(run_id: str, theme: str = Query("dark")):
    ...
    if not run:
        raise HTTPException(status_code=404, detail="Corrida no encontrada")
    summary = BASE_RUN_SUMMARIES.get(run_id)
    return HTMLResponse(_run_ai_report_html(run, summary, theme=theme))
