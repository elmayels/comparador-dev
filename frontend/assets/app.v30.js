...
function comparisonResults(){
  ...
  if(real){
    ...
    const rows = providerSummary.length ? providerSummary.map((p,i)=>[i+1, p.name, fmtMoney(p.total_amount), fmtMoney(p.market_total_amount), fmtMoney(p.overcost_amount), `${Number(p.overcost_pct||0).toFixed(1)}%`, p.reference_items, p.fallback_items, p.no_reference_items]) : providers.map((p,i)=>[i+1, p.name, p.concepts, p.apuItems, p.conceptsFile, p.matrixFile, '<span class="badge warn">Pendiente summary</span>']);
    shell(`${pageHead('Resultados de comparación', single?'Lectura individual contra mercado, sin ranking artificial.':'Ranking económico y diagnóstico contra mercado.', `<a class="btn btn-primary" href="${download}">Descargar Excel</a><a class="btn btn-secondary" href="${aiReport}" target="_blank">Ver diagnóstico profesional</a><button class="btn btn-secondary" data-nav="matrix-detail">Ver detalle APU</button>`)}${kpis([{label:'Corrida',value:real.id,text:'Datos reales'}, {label:'Tipo',value:single?'1 proveedor':`${summary.providersCount||providers.length} proveedores`,text:single?'Sin ranking':'Con ranking'}, {label:'Monto mínimo',value:fmtMoney(summary.minAmount),text:summary.bestProvider||'—'}, {label:'Brecha',value:`${Number(summary.economicSpreadPct||0).toFixed(1)}%`,text:single?'No aplica':'Entre extremos'}])}<div class="callout" style="margin-top:16px"><strong>Diagnóstico profesional:</strong> abre la vista de resultados para ver KPIs, tablas de evidencia, desglose por secciones y alertas contra mercado. En corrida de un solo proveedor no se fuerza ranking entre contratistas.</div><div style="margin-top:16px">${providerSummary.length?table(['#','Proveedor','Monto','Mercado','Diferencia','%','Referencias','Sin referencia plena','Sin referencia'], rows):table(['#','Proveedor','Conceptos','Filas APU','Archivo conceptos','Archivo matriz/APU','Estado'], rows)}</div>`, 'Resultados');
    return;
  }
  shell(`${pageHead('Resultados de comparación', multi?'Ranking económico y tablero ejecutivo.':'Análisis individual sin ranking económico.', `<a class="btn btn-primary" href="${reportUrl()}">Descargar Excel resultado</a><button class="btn btn-secondary" data-nav="matrix-detail">Ver detalle APU</button>`)}${kpis(multi?[{label:'Mejor oferta',value:money(1180000),text:(state.comparisonContractors[1]?.name || state.comparisonContractors[0]?.name || 'Proveedor')}, {label:'Riesgo global',value:'Amarillo',text:'Con advertencias'}, {label:'Críticas',value:'18',text:'A revisar'}, {label:'Sin referencia',value:'14',text:'Catálogos granulares'}]:[{label:'Monto ofertado',value:money(1250000),text:(state.comparisonContractors[0]?.name || 'Proveedor')}, {label:'Desv. referencia',value:'+12.3%',text:'Sin ranking'}, {label:'Críticas',value:'18',text:'A revisar'}, {label:'Semáforo',value:'Amarillo',text:'Riesgo medio'}])}<div class="grid cols-2" style="margin-top:16px"><div class="card"><h3>Hallazgos profesionales</h3><p>El sobrecosto se concentra en concreto, acero e instalaciones. Se recomienda revisar primero partidas con mayor impacto monetario, no solo las de mayor desviación porcentual.</p></div><div class="card"><h3>Recomendaciones</h3><p>Solicitar desglose APU, validar rendimientos, revisar precios de maquinaria fuera de referencia y negociar porcentajes superiores al rango esperado.</p></div></div><div style="margin-top:16px">${multi?table(['Ranking','Proveedor','Monto','Dif vs menor','Desv. promedio','Semáforo','Acciones'], [['1',n2,money(1180000),'0.0%','-7.8%','<span class="badge ok">Verde</span>','Ver detalle'],['2',n1,money(1250000),'+5.9%','+2.4%','<span class="badge warn">Amarillo</span>','Ver detalle'],['3',n3,money(1410000),'+19.5%','+14.3%','<span class="badge bad">Rojo</span>','Ver detalle']]):table(['Proveedor','Monto','Desv. referencia','Partidas críticas','Semáforo','Observación'], [[n1,money(1250000),'+12.3%','18','<span class="badge warn">Amarillo</span>','Sin ranking por ser análisis individual']])}</div>`, 'Resultados');
}
