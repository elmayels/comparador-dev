...
function comparisonResults(){
  ...
  if(real){
    ...
    shell(`${pageHead('Resultados de comparación', 'Reporte generado desde archivos reales y modelo de análisis.', `<a class="btn btn-primary" href="${download}">Descargar Excel real</a><button class="btn btn-secondary" data-nav="matrix-detail">Ver detalle APU</button>`)}${kpis([{label:'Corrida',value:real.id,text:'Datos reales'}, {label:'Proveedores',value:providers.length,text:'Con nombre corto'}, {label:'Conceptos leídos',value:providers.reduce((a,p)=>a+(p.concepts||0),0),text:'Catálogos'}, {label:'Filas APU',value:providers.reduce((a,p)=>a+(p.apuItems||0),0),text:'Matrices'}])}<div class="callout" style="margin-top:16px"><strong>Alcance:</strong> los archivos ya se leen y se procesan para generar la comparativa profesional. La homologación avanzada de conceptos y el matching semántico fino contra mercado quedan como siguiente iteración.</div><div style="margin-top:16px">${table(['#','Proveedor','Conceptos','Filas APU','Archivo conceptos','Archivo matriz/APU','Estado'], rows)}</div>`, 'Resultados');
    return;
  }
